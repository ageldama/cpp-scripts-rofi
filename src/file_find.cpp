...
directory_entry_pred make_only_file_and_regex_match(const string_vector regexes)
{
  ...
  return [regex_ptrs](const std::filesystem::directory_entry& entry) -> bool {
    if(!fs::is_regular_file(entry)) return false;
    std::string abs_path = entry.path().string();
    for(const auto &regex_ptr : regex_ptrs) {
      if (regexec(regex_ptr.get(), abs_path.c_str(),0, nullptr, 0)==0) return true;
    }
    return false;
  };
}
...
string_vector find_files_in_directories(const string_vector& dirs, directory_entry_pred entry_pred)
{
