  ...
  for(size_t i_cmd = 0 ; i_cmd < tot_cmd ; i_cmd++)
    {
      ...
      fread(static_cast<void*>(&tot_run_type_counts), sizeof(tot_run_type_counts), 1, fp);

      for(size_t i_run_type_count=0; i_run_type_count < tot_run_type_counts; i_run_type_count++)
        {
          run_count_t run_count = 0;
          fread(static_cast<void*>(&run_count), sizeof(run_count), 1, fp);
